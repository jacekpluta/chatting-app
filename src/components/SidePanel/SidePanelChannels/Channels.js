...
import { Menu, Icon, Modal, Button, Form, Input, Message, Label } from 'semantic-ui-react';
...
function Channels(props) {
	...
	const isFormIsValid = () => {
		if (channelName.length < 3 || channelName.length > 21) {
			setError('Channel name or channel detail is too short or too long (atleast 3, maximum 20 characters)');
			return false;
		} else if (channelDetail.length > 40) {
			setError('Channel name or channel details is too long (maximum 15 characters for name and 40 for details)');
			return false;
		} else {
			return true;
		}
		...
		() => {
			if (isPrivateChannel) {
				if (prevChannelId && prevChannelId.length < 21) {
					channelsRef.child(prevChannelId).child('usersInChannel').child(currentUser.uid).remove((err) => {
						if (err !== null) {
							console.log(err);
						}
					});
	...
	const currentUsersInChannel = (channel) => {
		...
		if (!isPrivateChannel && prevChannelId && channel && prevChannelId !== channel.id) {
			channelsRef.child(channel.id).child('usersInChannel').child(currentUser.uid).set(user).catch((error) => {
				console.log(error);
			});
	...
	const currentChannelUsersListener = (channel) => {
		channelsRef.child(channel.id).child('usersInChannel').on('child_added', (snapshot) => {
			if (currentUser.uid !== snapshot.key) {
				setUsersInCurrentChannel((users) => [ ...users, snapshot.val() ]);
			}
		});

		channelsRef.child(channel.id).child('usersInChannel').on('child_removed', (snapshot) => {
			if (currentUser.uid !== snapshot.key) {
	...
	useEffect(
		() => {
			if (usersInCurrentChannel) {
				const uniqueUsers = Array.from(new Set(usersInCurrentChannel.map((a) => a.id))).map((id) => {
	...
	const displayFavouritedChannels = () => {
		if (favChannels) {
			return favChannels.sort((a, b) => (a.name > b.name ? 1 : -1)).map((channel) => (
				<Menu.Item
					key={channel.id}
	...
	const displayChannels = () => {
		if (allChannels) {
			...
			const onlyPublicChannels = allChannels.filter((x) => !favChannels.filter((y) => y.id === x.id).length);

			return onlyPublicChannels.sort((a, b) => (a.name > b.name ? 1 : -1)).map((channel) => (
				<Menu.Item
					key={channel.id}
					onClick={() => changeChannel(channel)}
					name={channel.name}
				...
				>
					...
					getNotificationCount(channel) && <Label color="orange">{getNotificationCount(channel)}</Label>}
	...
	return (
		<React.Fragment>
			...
			<Menu.Item>
				<span style={{ color: 'white' }}>
					<Icon name="comment alternate" /> CHANNELS {'  '}(
					{allChannels !== undefined && allChannels.length})
				</span>

				<Icon onClick={handleOpenModal} style={{ color: 'white' }} name="add" />
			</Menu.Item>
			...
			<Modal open={modal} onClose={handleCloseModal} size="small">
				...
				<Modal.Content>
					<Form onSubmit={handleSubmit}>
						<Form.Field>
							<Input fluid label="Name of the channel" name="channelName" onChange={handleChange} />
						</Form.Field>
						<Form.Field>
							<Input fluid label="About the channel" name="channelDetalis" onChange={handleChange} />
						</Form.Field>
					</Form>
					{error !== '' && (
						<Message error>
				...
				<Modal.Actions>
					...
					<Button disabled={loading} onClick={handleCloseModal} className={'cancel'} color="red">
						Cancel
					</Button>
				</Modal.Actions>
			</Modal>
