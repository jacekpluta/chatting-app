...
import { Menu, Icon, Image, Popup, Button, List, Label } from 'semantic-ui-react';
import { connect } from 'react-redux';
import { setCurrentChannel, setPrivateChannel, setActiveChannelId, setChannelFriended } from '../../../actions';
...
const Friends = (props) => {
	...
	const addUsersFriendsListener = () => {
		usersRef.child(currentUser.uid).child('friends').on('child_added', function(snap) {
	...
	const addNotificationListener = (channelId) => {
		privateMessagesRef.child(getChannelId(channelId)).on('value', (snapshot) => {
			handleNotifications(channelId, currentChannel.id, notifications, snapshot);
		});
	};
	...
	const clearNotifications = (friendChannelId) => {
		if (currentUser) {
			let index = notifications.findIndex((notification) => notification.id === friendChannelId);
	...
	const addListenersFriendAdded = (userId) => {
		usersRef.child(userId).child('friends').on('child_added', (snapshot) => {
			if (currentUser.uid !== snapshot.key) {
				const friendToAdd = { id: snapshot.key, ...snapshot.val() };

				setFriendsChannels((friendedChannels) => [ ...friendedChannels, friendToAdd ]);
			}
		});
	};

	const addListenersPendingFriend = (userId) => {
		usersRef.child(userId).child('pendingFriends').on('child_added', (snapshot) => {
			setPendingFriends((pendingFriends) => [ ...pendingFriends, snapshot.val() ]);
		});
	};

	const addListenersPendingFriendRemoved = (userId) => {
		usersRef.child(userId).child('pendingFriends').on('child_removed', (snapshot) => {
			setFriendPendingToRemove(snapshot.key);
		});
	};

	const addListenersFriendRemoved = (userId) => {
		usersRef.child(userId).child('friends').on('child_removed', (snapshot) => {
	...
	useEffect(
		() => {
			if (friendPendingToRemove) {
				const filteredPendingFriend = pendingFriends.filter((friendPendingChannel) => {
					if (friendPendingChannel.id !== friendPendingToRemove) return friendPendingChannel;
					else {
						return null;
					}
				});
	...
	const getChannelId = (userId) => {
		...
		return userId < currentUserId ? `${userId}/${currentUser.uid}` : `${currentUser.uid}/${userId}`;
	};
	...
	const handleRemovePending = (pendingId) => {
		usersRef.child(`${currentUser.uid}/pendingFriends`).child(pendingId).remove((err) => {
			if (err !== null) {
				console.log(err);
			}
		});
	...
	const displayFriendChannels = () => {
		if (usersList && friendsChannels) {
			const friendsList = usersList.filter((o1) => friendsChannels.some((o2) => o1.id === o2.id));

			return friendsList.sort((a, b) => (a.name > b.name ? 1 : -1)).map((friendChannel) => (
				<Menu.Item
				...
				>
					<Icon name="circle" color={friendChannel && friendChannel.status ? 'green' : 'red'} />

					{friendChannel ? (
						<Image src={friendChannel.photoURL} style={{ width: '10%', height: '10%' }} avatar />
					) : (
						''
					)}
	...
	const uniquePendings = () => {
		...
		const onlyPendings = pendingFriends.filter((x) => !friendsChannels.filter((y) => y.id === x.id).length);

		//deletes duplicates
		const uniquePendings = Array.from(new Set(onlyPendings.map((a) => a.id))).map((id) => {
	...
	const displayPendingFriends = () => {
		if (uniqueFriendsPendings) {
			return uniqueFriendsPendings.sort((a, b) => (a.name > b.name ? 1 : -1)).map((pendingFriend) => (
				<List>
					<List.Item>
						<Image src={pendingFriend.photoURL} style={{ width: '10%', height: '10%' }} avatar />
						<List.Content>
							<List.Header as="a">
								{' '}
								<h4
		...
		<React.Fragment>
			<Menu.Item>
				<span style={{ color: 'white', paddingBottom: '5px' }}>
					<Icon name="users" /> FRIENDS ({friendsChannels.length}){'   '}
				</span>
				...
				{uniqueFriendsPendings.length !== 0 ? (
					<Popup
						...
						hoverable
						trigger={
							<Button color="orange" size="mini" compact style={{ color: 'white' }}>
								<Icon name="exclamation" /> ({uniqueFriendsPendings.length}
								)
							</Button>
						}
					>
