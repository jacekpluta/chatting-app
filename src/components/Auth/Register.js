...
class Register extends Component {
	...
	isPasswordValid = () => {
		if (this.state.password.length < 6 || this.state.passwordConfirmation.length < 6) {
			...
			return true;
		} else if (this.state.password.length !== this.state.passwordConfirmation.length) {
					...
					<Form size="large" onSubmit={this.handleSubmit}>
						<Segment stacked>
							...
							<Form.Input
								...
								onChange={this.handleChange}
								value={email}
								className={this.state.error.toLowerCase().includes('email') ? 'error' : ''}
							/>
							...
							<Form.Input
								...
								onChange={this.handleChange}
								value={password}
								className={this.state.error.toLowerCase().includes('password') ? 'error' : ''}
							/>
