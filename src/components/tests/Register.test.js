...
it('should find inputs with their correct names', () => {
	...
	expect(wrapper.find('input').at(3).prop('name')).toEqual('passwordConfirmation');
});
...
it('has a email text area that users can type in', () => {
	wrapper.find('input').at(1).simulate('change', { target: { name: 'email', value: 'test@o2.pl' } });
	wrapper.update();
...
it('has a passwordConfirmation text area that users can type in', () => {
	wrapper.find('input').at(1).simulate('change', { target: { name: 'passwordConfirmation', value: 'test123' } });
	wrapper.update();
